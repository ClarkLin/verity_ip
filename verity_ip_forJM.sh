    ...
    while read IP; do
        ...
        if [[ ! -n "$RESULT" ]]; then
        	GEOIP_LOCATION_INFO=`geoiplookup $IP | sed 'N;s/\n/:/' | cut -f2 -d ':'| sed 's/\,/;/'`
       		...
       		HOST_INFO=`host $IP`
       		USER_AGENT_INFO=`cat $ACCESSLOG | grep "Apr\/2015" | grep $IP |cut -d '"' -f 6 | sort -u`
       		MORE_INFO=`cat $ACCESSLOG | grep "Apr\/2015" | grep $IP | sort -u | cut -d '[' -f2 | sed 's/$/&<br\ \/>/g'`
...
function Main() {
	...
	CreateHtmlFile
	CopyResult
	sed -i "s/\,\;/\;/g" `grep ",;" -rl /usr/local/nginx/conf/block_ip/`
